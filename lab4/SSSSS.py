...
def TEST_normalize2dpts(in_pts):
...
TEST_norm_x, TEST_T1 = TEST_normalize2dpts(h_x.T)
TEST_Tnorm_xp, TEST_T2 = TEST_normalize2dpts(h_xp.T)



#print("h_x", np.shape(h_x), "h_x end")

"""
for iiii in range(3):
    for jjjj in range(618):
    	print(h_x[iiii][jjjj], end = " ")
    print("")
"""
'''
目前還是暫時用cv function
...
TEST_F, TEST_idx = RSC8pt.TEST_ransac_8points(TEST_norm_x.T, TEST_Tnorm_xp.T, TEST_T1, TEST_T2)
...
"""
(xxx, yyy) = np.shape(points3D)
for iiii in range(xxx):
    for jjjj in range(yyy):
        print(points3D[iiii][jjjj], end = " ")
    print("")
"""
print("points3D", np.shape(points3D), "3D end")
